<script setup>
    ...
    let getClient = () => {
        ...
        axios.get('/api/v1/clients/${clientID}')
        .then(response => {
    ...
    const unpaidInvoices = computed(() => {
        return client.invoices.filter(invoice => invoice.is_paid === false);
    });

    const paidInvoices = computed(() => {
        return client.invoices.filter(invoice => invoice.is_paid === true);
    });
